...
%
%
where $\Delta W_{si}^n = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}_s^p$ is the mechanical work rate computed from the constitutive model with $\pmb{\epsilon}_s^p$ is the plastic strain. The nodal internal temperature is calculated by
\begin{equation}
...
%
\begin{equation}
     \frac{1}{V}\frac{D_s(m_s e_s)}{Dt} = \Delta W_{s} + \nabla \cdot \pmb{q}_s -  \sum q_{sf} 
\end {equation}
%
%
On the right hand side, the terms include the rate of the mechanical work $\Delta W_{s} = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}_s^p$ computed from the constitutive model with $\pmb{\epsilon}_s^p$ is the plastic strain, thermal transport and energy exchange between solid and fluid respectively. The heat flux is $\pmb{q}_s = \overline{\rho}_s \beta_s \nabla T_s$ with $\beta_s$ being the thermal conductivity of the solid materials
%
%
