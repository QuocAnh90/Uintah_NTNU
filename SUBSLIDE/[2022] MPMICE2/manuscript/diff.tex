...
%DIF ADD MPMICE_v3.tex   Tue Sep 13 10:13:43 2022
...
%
%
\DIFdelbegin \DIFdel{Solving the equation of state (see Appendix's section 'Evolution of specific volume') to the rate of the equilibrium pressure of fluid mixture as:
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation}
%DIFDELCMD < %%%
\DIFdel{\sum_{f=1}^{N_f} \phi_f \kappa_f \frac{D_f P_{eq}}{Dt} = - \nabla \cdot }%DIFDELCMD < \pmb{U} %%%
\DIFdel{+ \sum_{f=1}^{N_f} \phi_f \alpha_f \frac{D_f T_f}{Dt}
}%DIFDELCMD < 
\end {equation}
%DIFDELCMD < %%%
%DIF < 
...
\begin{equation}
%DIFDELCMD < \label{specific volume}
%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f \frac{D_f \upsilon_f }{Dt} = f_f^{\phi} \nabla \cdot }%DIFDELCMD < \pmb{U} %%%
\DIFdel{+ (\phi_f \alpha_f \frac{D_f T_f}{Dt} - f_f^{\phi} \sum_{n=1}^{N} \phi_n \alpha_n \frac{D_n T_n}{Dt})
}%DIFDELCMD < 
\end {equation}
%DIFDELCMD < %%%
\DIFdel{where $ f_f^{\phi} = (\phi_f  \kappa_f ) / (\sum_{n=1}^{N} \phi_n \kappa_n)$ with  $\kappa_f$ being f-material bulk compressibility and $\alpha$ is the constant pressure thermal expansitivity of fluids.
%DIF < 
%DIF < 
...
Currently, the energy exchange coefficient $H_{sf}$ is assumed to be constant for the sake of simplicity. In all simulations, we assume \DIFdelbegin \DIFdel{$H_{sf}=100$}\DIFdelend \DIFaddbegin \DIFadd{$H_{sf}=1$}\DIFaddend .  Then the energy exchange can be written as:
%
%
\begin{equation}
     q_{sf} = H_{sf} (T_f -T_s)
...
%
%
\DIFaddbegin \DIFadd{On the other hand, the draf force can be calculated as:
%DIF > 
%DIF > 
}
\begin{equation}
...
%
%
\DIFdelbegin \subsection{\textsf{\DIFdel{Evolution of specific volume}}%DIFAUXCMD
}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIF < 
%DIF < 
\DIFdel{The restriction for the equilibrium pressure can be derived from:
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation}
...
\begin{gathered}
%DIFDELCMD <     %%%
\DIFdel{0 = \frac{\partial }{\partial t} (n - \sum_{f=1}^{N_f} \upsilon_f }%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f) =  \frac{\partial n}{\partial t} - \sum_{f=1}^{N_f} (}%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f \frac{\partial \upsilon_f}{\partial t} + \upsilon_f \frac{\partial \overline{\rho}_f}{\partial t}) }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{= \frac{\partial n}{\partial t} - \sum_{f=1}^{N_f} (}%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f \frac{D_f \upsilon_f}{Dt} + \nabla \cdot (\upsilon_f }%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_f))}%DIFDELCMD < \\
...
%DIF < 
%DIF < 
\DIFdel{Putting the equation (\ref{fluidvolumerate}) into equation (\ref{equilP}) leading to
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation} 
...
\label{equilP1}
%DIFDELCMD <     %%%
\DIFdel{0= \frac{\partial n}{\partial t} - \sum_{f=1}^{N_f} (}%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f \upsilon_f (\kappa_f \frac{D_f P_{eq}}{Dt} + \alpha_f  \frac{D_f T_f}{Dt}) + \nabla \cdot (\upsilon_f }%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_f))}%DIFDELCMD < \\
%DIFDELCMD < 
\end {equation}
...
%DIF < 
%DIF < 
\DIFdel{Putting equation (\ref{p1}) into equation (\ref{equilP1}) leads to:
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation} 
...
%DIF < 
%DIF < 
\DIFdel{where $\pmb{U} = \sum_{s=1}^{N_s} \phi_s \pmb{U}_s + \sum_{f=1}^{N_f}  \phi_f \pmb{U}_f$. And the rate of the specific volumne is computed by putting equation (\ref{DP}) into equation (\ref{specificvolume}):
}%DIFDELCMD < 
\begin{equation}
%DIFDELCMD < \overline{\rho}%%%
\DIFdel{_f \frac{D_f \upsilon_f }{Dt} = f_f^{\phi} \nabla \cdot }%DIFDELCMD < \pmb{U} %%%
...
\end {equation}
%DIFDELCMD < %%%
\DIFdel{where $ f_f^{\phi} = (\phi_f  \kappa_f ) / (\sum_{n=1}^{N} \phi_n \kappa_n)$ with  $\kappa_f$ being f fluid material's bulk compressibility.
}%DIFDELCMD < 
...
%
\begin{equation}
     \frac{1}{V}\frac{D_s(m_s e_s)}{Dt} = \DIFdelbegin %DIFDELCMD < \pmb{\sigma}%%%
\DIFdel{^\prime:\nabla }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_s
    }\DIFdelend \DIFaddbegin \DIFadd{\Delta W_{s} }\DIFaddend + \nabla \cdot \pmb{q}_s -  \DIFaddbegin \DIFadd{\sum }\DIFaddend q_{sf} 
\end {equation}
%
%
On the right hand side, the terms include the mechanical work \DIFaddbegin \DIFadd{$\Delta W_{s} = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}^p_s$ computed from the constitutive model with $\pmb{\epsilon}^p_s$ is the plastic strain}\DIFaddend , thermal transport and energy exchange between solid and fluid respectively. The heat flux is $\pmb{q}_s = \overline{\rho}_s \beta_s \nabla T_s$ with $\beta_s$ being the thermal conductivity of the solid materials
%
%
