...
%DIF DEL MPMICE_v2.tex   Tue Sep 13 05:17:49 2022
%DIF ADD MPMICE_v4.tex   Fri Oct 21 13:36:32 2022
...
%DIF < \journal{Computer Methods in Applied Mechanics and Engineering          }
%DIF -------
\journal{Arxiv} %DIF > 
...
\title{MPMICE: A \DIFdelbegin \DIFdel{Lagrangian-Eulerian framework of multiphase flows }\DIFdelend \DIFaddbegin \DIFadd{hybrid MPM-CFD model for simulating coupled problems }\DIFaddend in \DIFdelbegin \DIFdel{deformable }\DIFdelend porous media\DIFaddbegin \DIFadd{. Application }\DIFaddend to \DIFdelbegin \DIFdel{model }\DIFdelend earthquake-induced submarine landslides}
...
\DIFdelbegin \DIFdel{$\pmb{U}_s$   			}%DIFDELCMD < &  	%%%
\DIFdel{$[Lt^{-1}]$    	}%DIFDELCMD < & %%%
\DIFdel{Solid Velocity gradient vector}%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelend $\pmb{a}_s$   			&  	$[Lt^{-2}]$   	& Solid Acceleration vector\\
$\pmb{\sigma}^\prime$ 	&  	\DIFdelbegin \DIFdel{$[ML^{-2}]$ 	}\DIFdelend \DIFaddbegin \DIFadd{$[ML^{-1}t^{-2}]$ 	}\DIFaddend & Effective Stress tensor\\
$ \pmb{\epsilon}_s$ 	&  			 	& Strain tensor\\
$e_s$         				&  	$[L^2t^{-2}]$  	& Solid Internal energy \DIFaddbegin \DIFadd{per unit mass}\DIFaddend \\   
...
$e_f$         				&  $[L^2t^{-2}]$  		& Fluid Internal energy \DIFaddbegin \DIFadd{per unit mass}\DIFaddend \\   
...
\item Solid grains are incompressible while the fluid phases are compressible. \DIFaddbegin \DIFadd{Solid's thermal expasion is negligible.
}\DIFaddend \item There is no mass exchange between solid and fluid phases.
...
\end {equation}
%
\DIFaddbegin \DIFadd{Solving the mass balance equation leads to:
%DIF > 
%DIF > 
}\begin{equation}
  \DIFadd{\frac{D_s n}{Dt} = \phi_s \nabla \cdot }\pmb{U}\DIFadd{_s  
}\end {equation}
%DIF > 
%DIF > 
...
%
\begin{equation}
     \frac{1}{V} \left[ \frac{\partial (m_f \pmb{U}_f)}{\partial t} + \nabla \cdot  (m_f \pmb{U}_f \pmb{U}_f) \right] = - \phi_f \nabla p_f  +  \nabla \cdot \pmb{\tau}_f + \overline{\rho}_f \pmb{b} +  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{d}
\end {equation}
%
%
The momentum balance equation for the solid \DIFdelbegin \DIFdel{phase is}\DIFdelend \DIFaddbegin \DIFadd{phases are}\DIFaddend :\\
%
%
\begin{equation}
     \frac{1}{V}\frac{D_s(m_s \pmb{U}_s)}{Dt} = 
    \nabla \cdot (\pmb{\sigma}^\prime) - \phi_s \nabla p_f 
    + \overline{\rho}_s \pmb{b} +  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{fric}    -  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{d}
\end {equation}
%
%
%_______________________________
...
\begin{equation}
    ...
    -\overline{\rho}_f p_f  \frac{D_f\upsilon_f}{Dt} + \pmb{\tau}_f : \nabla \pmb{U}_f + \nabla \cdot \pmb{q}_f +  \DIFaddbegin \DIFadd{\sum }\DIFaddend q_{sf}
\end {equation}
%
%
...
\begin{equation}
    \label{solidenergy}
     \DIFdelbegin \DIFdel{\frac{1}{V}\frac{D_s(m_s e_s)}{Dt} }\DIFdelend \DIFaddbegin \DIFadd{\frac{m_s}{V}  c_v \frac{D_s( T_s)}{Dt} }\DIFaddend = \pmb{\sigma}^\prime: \DIFdelbegin \DIFdel{\nabla }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_s }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_s( \pmb{\epsilon}_s^p)}{Dt} }\DIFaddend + \nabla \cdot \pmb{q}_s -  \DIFaddbegin \DIFadd{\sum }\DIFaddend q_{sf} 
\end {equation}
%
%
\DIFaddbegin \DIFadd{where $c_v$ is the specific heat at constant volume of the solid materials.
}\DIFaddend \underline{\hspace{5in}}\\
...
(3) Frictional \DIFdelbegin \DIFdel{force }\DIFdelend \DIFaddbegin \DIFadd{forces }\DIFaddend $\pmb{f}_{fric}$ for the contact \DIFdelbegin \DIFdel{between }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend soil-structure interaction \DIFaddbegin \DIFadd{between solid/porous materials }\DIFaddend with the friction coefficient $\mu_{fric}$. \\
(4) Exchange momentum \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{models }\DIFaddend (computing drag force $\pmb{f}_{d}$) \DIFaddbegin \DIFadd{for interaction between materials}\DIFaddend . \\
(5) Energy exchange \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{models }\DIFaddend (computing temerature exhange term $q_{sf}$) \DIFaddbegin \DIFadd{for interaction between materials}\DIFaddend . \\
(6) An equation of state to establish relations between thermodynamics variables of each fluid materials \DIFdelbegin \DIFdel{$[P_{eq}, \overline{\rho}_f, \upsilon_f, T_f, e_f]$}\DIFdelend \DIFaddbegin \DIFadd{$[P_{f}, \overline{\rho}_f, \upsilon_f, T_f, e_f]$}\DIFaddend . \\
\DIFdelbegin \DIFdel{Here, $P_{eq}$ is "equilibrium" pressure which is defined as pressure enables mass of fluid material to fills in an entire pore volume with no ongoing compression or expansion with given true density $\rho_f$ and temperature $T_f$.  }\DIFdelend Four thermodynamic relations for the equation of states are:
%
%
\begin{equation}
\begin{gathered}
  e_f =  e_f (T_f, \upsilon_f)\\
 \DIFdelbegin \DIFdel{\upsilon}\DIFdelend \DIFaddbegin \DIFadd{P}\DIFaddend _f =  \DIFdelbegin \DIFdel{\upsilon}\DIFdelend \DIFaddbegin \DIFadd{P}\DIFaddend _f(T_f, \DIFdelbegin \DIFdel{P_{eq}}\DIFdelend \DIFaddbegin \DIFadd{\upsilon_f}\DIFaddend )\\
...
\end {gathered}
\end {equation}
\DIFaddbegin \DIFadd{Frictional sliding between solid materials also generates dissipation and the work rate generated from the sliding can be calculated as:
%DIF > 
}\begin{equation}
   \DIFadd{\Delta W_{friction} = }\pmb{f}\DIFadd{_{fric} d
}\end {equation}
%DIF > 
%DIF > 
\DIFadd{where d is the sliding distance which can computed based on the sliding velocity between two materials. 
}\DIFaddend %_______________________________
\subsubsection{Momentum and Energy exchange model}
Currently, the energy exchange coefficient $H_{sf}$ is assumed to be constant for the sake of simplicity. \DIFdelbegin \DIFdel{In all simulations, we assume $H_{sf}=100$.  }\DIFdelend Then the energy exchange can be written as:
%
%
\begin{equation}
     q_{sf} = H_{sf} (T_f -T_s)
...
%
%
\DIFaddbegin \DIFadd{On the other hand, the drag force can be calculated as:
%DIF > 
%DIF > 
}\begin{equation}
    \DIFadd{f_{d} = K (}\pmb{U}\DIFadd{_s - }\pmb{U}\DIFadd{_f)  
}\end {equation}
%DIF > 
%DIF > 
\DIFaddend For the momentum exchange \DIFaddbegin \DIFadd{between fluid flows and porous media}\DIFaddend , we assume that the drag force $\pmb{f}_{d}$ depends on the average grain size of the grains $D_p$, the porosity $n$, the fluid vicosity $\mu_f$, and is propotional to the relative velocities of soil grains and fluid $(\pmb{U}_s - \pmb{U}_f)$. Based on recent investigation of CFD simulations of fluid flow around mono- and bi-disperse packing of spheres for $0.1 < \phi_s < 0.6$ and $Re < 1000$ \cite{Drag}. The drag force is given by: \\
%
%
\begin{equation}
...
\begin{enumerate}
\item \DIFdelbegin \DIFdel{For soil-structure interaction, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend drag force is set to \DIFdelbegin \DIFdel{be zero and }\DIFdelend \DIFaddbegin \DIFadd{zero in soil-structure interactions, and instead }\DIFaddend the frictional force is computed\DIFdelbegin \DIFdel{instead}\DIFdelend .
\item \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{As a result of }\DIFaddend fluid-structure interaction, the momentum exchange coefficient \DIFdelbegin \DIFdel{is set to }\DIFdelend \DIFaddbegin \DIFadd{should }\DIFaddend be extremely high \DIFdelbegin \DIFdel{which consider }\DIFdelend \DIFaddbegin \DIFadd{when }\DIFaddend the solid material points \DIFdelbegin \DIFdel{being the '}\DIFdelend \DIFaddbegin \DIFadd{are considered to be }\DIFaddend zero-porosity\DIFdelbegin \DIFdel{'}\DIFdelend /\DIFdelbegin \DIFdel{'zero permeability' materials}\DIFdelend \DIFaddbegin \DIFadd{zero-permeability}\DIFaddend .
\item \DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{In the case of }\DIFaddend soil-fluid interaction, the drag force is \DIFdelbegin \DIFdel{computed based on }\DIFdelend \DIFaddbegin \DIFadd{calculated using }\DIFaddend the equation (\ref{fd}). \DIFdelbegin \DIFdel{Since air has much lower vicosity }\DIFdelend \DIFaddbegin \DIFadd{Considering that air has a much lower viscosity }\DIFaddend than water, \DIFdelbegin \DIFdel{the drag force of air }\DIFdelend \DIFaddbegin \DIFadd{its drag force }\DIFaddend is much lower than the drag force of water in \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend pore.
\item \DIFdelbegin \DIFdel{For between multiphase flows, the }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend momentum exchange coefficient \DIFdelbegin \DIFdel{is set to be }\DIFdelend \DIFaddbegin \DIFadd{of }\DIFaddend 1E5 \DIFdelbegin \DIFdel{. That value is sufficient to increase the numerical stability for the }\DIFdelend \DIFaddbegin \DIFadd{is applied between multiphase flows. This value is far higher than reality \mbox{%DIFAUXCMD
\cite{Stewart1974}}\hskip0pt%DIFAUXCMD
, but it is necessary to have enough numerical stability to conduct }\DIFaddend simulations in the numerical example.
\end {enumerate}
...
%
\begin{equation}
    0 = n - \sum_{f=1}^{N_f} \upsilon_f \overline{\rho}\DIFaddbegin \DIFadd{_f}\DIFaddend \\
\label{n}
\end {equation}
%
%
...
}\DIFdelend \DIFaddbegin \DIFadd{\frac{(\Delta W_{si}^n + \Delta W_{fric,i}^n + \nabla^i \cdot \pmb{q}_{si}^n)}{m_{si}^n c_v}
}\DIFaddend \end {equation}
%
%
where \DIFdelbegin \DIFdel{$\Delta W_{si}^n = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}^p_s$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta W_{si}^n = \pmb{\sigma}^\prime : \frac{D_s( \pmb{\epsilon}_s^p)}{Dt} $ }\DIFaddend is the mechanical work rate computed from the constitutive model with \DIFdelbegin \DIFdel{$\pmb{\epsilon}^p_s$ }\DIFdelend \DIFaddbegin \DIFadd{$\pmb{\epsilon}_s^p$ }\DIFaddend is the plastic strain\DIFdelbegin \DIFdel{. The nodal internal temperature is calculated by
}\DIFdelend \DIFaddbegin \DIFadd{, $ \Delta W_{fric,i}^n$ is the work rate compted from the contact law due to the frictional sliding between solid materials. The heat flux is $\pmb{q}_s = \overline{\rho}_s \beta_s \nabla T_s$ with $\beta_s$ being the thermal conductivity of the solid materials.
}
...
%
\begin{equation}
 (me)_{sc}^{L-} =  \sum{S_{ci} m_{si}^n T_{si}^L} \DIFaddbegin \DIFadd{c_v
}\DIFaddend \end {equation}
%
%
...
%
%
And the increment of the velocity \DIFaddbegin \DIFadd{$\pmb{U}_{fc}$, $\Delta \pmb{U}_{sc}$ }\DIFaddend can be computed by solving the linear equation with i,j = 1 : $N$ as:
%
%
\[ \begin{vmatrix} (1 + \beta_{ij})  &  -\beta_{ij} \\
                  -\beta_{ji}       &  (1 + \beta_{ji})
...
%
%
The state variables of the solid phase \DIFdelbegin \DIFdel{$[\pmb{U}_{sp}^{n+1},\pmb{x}_{sp}^{n+1},\nabla \pmb{U}_{sp}^{n+1}, T_{sp}^{n+1}, \pmb{F}_{sp}^{n+1},V_{sp}^{n+1}]$ }\DIFdelend \DIFaddbegin \DIFadd{$[\pmb{U}_{sp}^{n+1},\pmb{x}_{sp}^{n+1},\nabla \pmb{U}_{sp}^{n+1}, T_{sp}^{n+1},  \nabla T_{sp}^{n+1}, \pmb{F}_{sp}^{n+1},V_{sp}^{n+1}]$ }\DIFaddend (velocity, position, velocity gradient, temperature, \DIFaddbegin \DIFadd{temperature gradient, }\DIFaddend deformation gradient, volume) are updated here 
%
%
\begin{equation}
...
All input files and the analytical calculations in this section are provided in the Github repository \DIFdelbegin \footnote{\DIFdel{https://github.com/QuocAnh90/Uintah NTNU}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend for the reproduction of the numerical results.\\
%_______________________________
...
\begin{enumerate}
\item At the initial of the seismic event, the seismic loading triggers the first slide at 3 seconds. At 4 seconds, the debris start to move with the maximum speed of around 2-3 m/s with multiple shear band developed in the slope. The wave generated from the submarine slide is around 2-3m towards the slide \DIFdelbegin \DIFdel{direaction}\DIFdelend \DIFaddbegin \DIFadd{direction}\DIFaddend .
%
\begin{figure}[H]
\center
...
%
\begin{equation}
   \frac{D_sm_s}{Dt} = \DIFdelbegin \DIFdel{\frac{D_s((1-n) \rho_s V)}{Dt} }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_s(\phi_s \rho_s V)}{Dt} }\DIFaddend = \rho_s V \DIFdelbegin \DIFdel{\frac{D_s(1-n)}{Dt} }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_s \phi_s}{Dt} }\DIFaddend + \DIFdelbegin \DIFdel{(1-n) }\DIFdelend \DIFaddbegin \DIFadd{\phi_s }\DIFaddend V \frac{D_s \rho_s}{Dt} + \DIFdelbegin \DIFdel{(1-n) }\DIFdelend \DIFaddbegin \DIFadd{\phi_s }\DIFaddend \rho_s  \frac{D_s V}{Dt} = 0  
\end {equation}
%
%
...
%
\begin{equation}
  V \DIFdelbegin \DIFdel{\frac{D_s(1-n)}{Dt} }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_s \phi_s}{Dt} }\DIFaddend + \DIFdelbegin \DIFdel{(1-n) }\DIFdelend \DIFaddbegin \DIFadd{\phi_s }\DIFaddend \frac{D_s V}{Dt} = 0  
\end {equation}
%
%
...
\begin{equation}
  ...
  \DIFdelbegin \DIFdel{(1-n) \nabla \cdot }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_s }%DIFDELCMD < \end {%%%
\DIFdel{equation}%DIFDELCMD < }
%DIFDELCMD < %%%
%DIF < 
%DIF < 
\DIFdel{Finally, we get:
%DIF < 
%DIF < 
}%DIFDELCMD < \begin{equation}
%DIFDELCMD < \label{p1}
%DIFDELCMD < %%%
\DIFdel{\frac{\partial n}{\partial t} = (1-n) }\DIFdelend \DIFaddbegin \DIFadd{\phi_s }\DIFaddend \nabla \cdot \pmb{U}_s  
\DIFdelbegin \DIFdel{- }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_s \cdot \nabla n
}\DIFdelend \end {equation}
%
%
\DIFdelend \subsection{\textsf{Momentum conservation}}
\label{Momentum}
%
%
...
\begin{equation}
    ...
    \nabla \cdot (-\phi_f p_f\pmb{I}) + \nabla \cdot \pmb{\tau}_f + \overline{\rho}_f \pmb{b} +  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{d} + \pmb{f}_b
\end {equation}
%
%
...
\begin{equation}
    \label{solidmom}
     \frac{1}{V}\frac{D_s(m_s \pmb{U}_s)}{Dt} = 
    \nabla \cdot (\pmb{\sigma}^\prime) - \phi_s \nabla p_f 
    + \overline{\rho}_s \pmb{b}   -  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{d} +  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{fric}
\end {equation}
%
%
Here the $\pmb{f}_{fric}$ stems from the soil-structure interaction following the contact law \DIFdelbegin \DIFdel{in the soil-structure interace}\DIFdelend \DIFaddbegin \DIFadd{between the soil/structure interaces}\DIFaddend .
...
\begin{equation}
\label{e1}
     \frac{1}{V}\frac{D_f(m_f (e_f+0.5\pmb{U}_f^2))}{Dt} = \nabla \cdot (-np_f\pmb{I}) \cdot \pmb{U}_f + \nabla \cdot \pmb{q}_f + (\overline{\rho}_f \pmb{b}) \cdot \pmb{U}_f +  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{d} \cdot \pmb{U}_f +  \DIFaddbegin \DIFadd{\sum }\DIFaddend q_{sf}
\end {equation}
%
%
...
\begin{equation}
   ...
   \DIFdelbegin \DIFdel{-}\DIFdelend \DIFaddbegin \DIFadd{= }\DIFaddend \overline{\rho}_f p_f  \frac{D_f\upsilon_f}{Dt} + \nabla \cdot \pmb{q}_f +  \DIFaddbegin \DIFadd{\sum }\DIFaddend q_{sf}
\end {equation}
%
%
On the right hand side, the terms include the average pressure-volume work, the average viscous dissipation, the thermal transport and the energy exchange between solid and fluid respectively. The heat flux is $\pmb{q}_f = \overline{\rho}_f \beta_f \nabla T_f$ with $\beta_f$ being the thermal conductivity coefficient. To derive the internal energy balance equation for the solid phase, we \DIFdelbegin \DIFdel{begin with the }\DIFdelend \DIFaddbegin \DIFadd{introduce the rate of the internal energy  for the thermoelastic materials as a function of elastic strain tensor $\pmb{\epsilon}_s^e$ and temperature $T_s$:
%DIF > 
%DIF > 
}\begin{equation}
   \DIFadd{\frac{m_s}{V} \frac{D_s( e_s)}{Dt} = }\pmb{\sigma}\DIFadd{^\prime :\frac{D_s( \pmb{\epsilon}_s^e)}{Dt} + \frac{D_s( e_s)}{D_s (T_s)} \frac{D_s( T_s)}{Dt} = }\pmb{\sigma}\DIFadd{^\prime :\frac{D_s( \pmb{\epsilon}_s^e)}{Dt} + c_v \frac{D_s( T_s)}{Dt}
}\label{internalEnergy}
\end {equation}
%DIF > 
%DIF > 
\DIFadd{$c_v$ is the specific heat at the constant volume of the solid materials. The }\DIFaddend total energy balance equation for the mixture based on \cite{Hassanizadeh} \DIFaddbegin \DIFadd{can be written }\DIFaddend as:\\
%
%
\begin{equation}
\label{e3}
\begin{gathered}
     \frac{1}{V}\frac{D_f(m_f (e_f+0.5\pmb{U}_f^2))}{Dt} + \frac{1}{V}\frac{D_s(m_s (e_s+0.5\pmb{U}_s^2))}{Dt} = \nabla \cdot (\DIFdelbegin \DIFdel{-np_f }\DIFdelend \DIFaddbegin \DIFadd{-\phi_f p_f}\DIFaddend \pmb{I}) \cdot \pmb{U}_f \\
     + \nabla \cdot (\pmb{\sigma}^\prime-\DIFdelbegin \DIFdel{(1-n)}\DIFdelend \DIFaddbegin \DIFadd{\phi_s }\DIFaddend p_f\pmb{I}) \cdot \pmb{U}_s + (\DIFdelbegin \DIFdel{-np_f }\DIFdelend \DIFaddbegin \DIFadd{-\phi_f p_f}\DIFaddend \pmb{I}) : \nabla \pmb{U}_f + \DIFdelbegin \DIFdel{(}\DIFdelend \pmb{\sigma}^\prime \DIFdelbegin \DIFdel{-(1-n)p_f}%DIFDELCMD < \pmb{I}%%%
\DIFdel{) }\DIFdelend : \nabla \pmb{U}_s \\
     + (\overline{\rho}_f \pmb{b}) \cdot \pmb{U}_f + (\overline{\rho}_s \pmb{b}) \cdot \pmb{U}_s
     + \nabla \cdot \pmb{q}_f + \nabla \cdot \pmb{q}_s +  \DIFaddbegin \DIFadd{\sum }\DIFaddend \pmb{f}_{d} \cdot (\pmb{U}_f - \pmb{U}_s)  
\end {gathered}
\end {equation}
%
%
Subtracting equation (\ref{e3})\DIFaddbegin \DIFadd{, (\ref{internalEnergy}) }\DIFaddend to equations (\ref{e1}) and (\ref{solidmom}), we obtained the internal energy balance equation for solid phase as:
%
%
\begin{equation}
      \DIFdelbegin \DIFdel{\frac{1}{V}\frac{D_s(m_s e_s)}{Dt} = }\DIFdelend \pmb{\sigma}^\prime :\DIFdelbegin \DIFdel{\nabla }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_s
    }\DIFdelend \DIFaddbegin \DIFadd{\frac{D_s( \pmb{\epsilon}_s^e)}{Dt}  }\DIFaddend + \DIFaddbegin \DIFadd{\frac{m_s}{V} c_v \frac{D_s( T_s)}{Dt} = \Delta W_{s} +  \Delta W_{friction} + }\DIFaddend \nabla \cdot \pmb{q}_s -  \DIFaddbegin \DIFadd{\sum }\DIFaddend q_{sf} 
\end {equation}
%
%
On the right hand side, the terms include the \DIFdelbegin \DIFdel{mechanical work }\DIFdelend \DIFaddbegin \DIFadd{work rate from frictional sliding between solid materials  $\Delta W_{friction}$}\DIFaddend , thermal transport and energy exchange between solid and fluid respectively. The heat flux is $\pmb{q}_s = \overline{\rho}_s \beta_s \nabla T_s$ with $\beta_s$ being the thermal conductivity of the solid materials\DIFaddbegin \DIFadd{, the mechanical work rate $\Delta W_{s} = \pmb{\sigma}^\prime :\frac{D_s( \pmb{\epsilon}_s)}{Dt}  = \pmb{\sigma}^\prime : (\frac{D_s( \pmb{\epsilon}_s^e)}{Dt}  + \frac{D_s( \pmb{\epsilon}_s^p)}{Dt}) $ computed from the constitutive model with $\pmb{\epsilon}_s^p$ is the plastic strain tensor, . By subtracting the term $\pmb{\sigma}^\prime : \frac{D_s( \pmb{\epsilon}_s^e)}{Dt}$, we get the final form of the energy balance equation as:
%DIF > 
%DIF > 
}\begin{equation}
      \DIFadd{\frac{m_s}{V} c_v \frac{D_s( T_s)}{Dt} = }\pmb{\sigma}\DIFadd{^\prime : \frac{D_s( \pmb{\epsilon}_s^p)}{Dt} +  \Delta W_{friction} + \nabla \cdot }\pmb{q}\DIFadd{_s -  \sum q_{sf} 
}\end {equation}
\DIFaddend %
%
