...
%DIF DEL MPMICE_v5.tex   Fri Apr 28 10:22:05 2023
%DIF ADD MPMICE_v4.tex   Tue Apr 18 12:53:11 2023
...
\item ICE (compressible multi-material CFD formulation) allows \DIFdelbegin \DIFdel{stabilizing }\DIFdelend \DIFaddbegin \DIFadd{to stablize }\DIFaddend pore water pressure and turbulent flow.
...
The turbulent effect is modelled using a statistical approach namely large-eddy simulation. In this approach, the micro-scale turbulent influence in the dynamics of the macro-scale motion is computed through simple models like Smagorinsky model. In the Smagorinsky \DIFdelbegin \DIFdel{model}\DIFdelend \DIFaddbegin \DIFadd{mode}\DIFaddend , the residual stress tensor is:
%
\begin{equation}
...
%
%
where $C_s$ is the Smagorinsky constant \DIFdelbegin \DIFdel{with the value of 0.1 }\DIFdelend and $\triangle = \sqrt[3]{dx dy dz}$ is the grid size that defines the subgrid length scale.
%_______________________________
\subsubsection{Frictional force for soil-structure interaction}
MPMICE includes a \DIFdelbegin \DIFdel{contact law }\DIFdelend \DIFaddbegin \DIFadd{contactlaw }\DIFaddend for the interaction between soil and structure using the first Coulomb friction contact for MPM presented by Bardenhagen et al. (\cite{MPMcontact}).  The magnitude of the friction force at the contact depends on the friction coefficient $\mu_{fric}$ and the normal force  $ \pmb{f}_{norm}$  computed from the projection of the contact force  in the normal direction.
%
\begin{equation}
...
%
%
For the momentum exchange between fluid flows and porous media, we assume that the drag force $\pmb{f}_{d}$ depends on the average grain size of the grains $D_p$, the porosity $n$, the fluid \DIFdelbegin \DIFdel{viscosity }\DIFdelend \DIFaddbegin \DIFadd{vicosity }\DIFaddend $\mu_f$, and is \DIFdelbegin \DIFdel{proportional }\DIFdelend \DIFaddbegin \DIFadd{propotional }\DIFaddend to the relative velocities of soil grains and fluid $(\pmb{U}_s - \pmb{U}_f)$. Based on recent investigation of CFD simulations of fluid flow around mono- and bi-disperse packing of spheres for $0.1 < \phi_s < 0.6$ and $Re < 1000$ \cite{Drag}. The drag force is given by: \\
%
%
\begin{equation}
...
%
%
When validating the model with analytical solution, it requires to know the hydraulic conductivity\DIFdelbegin \DIFdel{$K$}\DIFdelend . In such case, we convert the equation (\ref{fd}) to Kozeny-Carman formula by assuming $F(\phi_s, Re) = 10\phi_s/(1-\phi_s)^2$, \DIFdelbegin \DIFdel{leading to 
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation}
%DIFDELCMD <      
\pmb{f}%%%
\DIFdel{_{d} = \frac{180 \phi_s^2\mu_f}{D_p^2 (1-\phi_s)} (}%DIFDELCMD < 
\pmb{U}%%%
\DIFdel{_s - }%DIFDELCMD < \pmb{U}%%%
\DIFdel{_f)  
}%DIFDELCMD < 
\label{fd}
%DIFDELCMD <
...
%DIF < 
%DIF < 
\DIFdel{Then, the draging force following the Darcy law is given by:
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation}
%DIFDELCMD <      
\pmb{f}%%%
\DIFdel{_{d} =  \frac{n^2 \mu_f}{ \kappa}(}%DIFDELCMD < 
\pmb{U}%%%
\DIFdel{_s - }%DIFDELCMD < 
\pmb{U}%%%
\DIFdel{_f)  
...
%DIF < 
%DIF < 
\DIFdel{where $\kappa$ being intrinsic permeability of soil which can be written as:
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation}
%DIFDELCMD <      %%%
\DIFdel{\kappa  =  \frac{K \mu_f}{\rho_f  g}
}%DIFDELCMD < 
\end {%%%
\DIFdel{equation}%DIFDELCMD < }
%DIFDELCMD < %%%
%DIF < 
%DIF < 
\DIFdel{As such, the }\DIFdelend \DIFaddbegin \DIFadd{then the }\DIFaddend hydraulic conductivity will be expressed as  \DIFdelbegin \DIFdel{:
%DIF < 
%DIF < 
}%DIFDELCMD < 
\begin{equation}
%DIFDELCMD <     %%%
\DIFdel{K = \frac{D_p^2 (1-\phi_s)^3}{180 \phi_s^2 \mu_f} \rho_f  g
}%DIFDELCMD < 
\end {%%%
\DIFdel{equation}%DIFDELCMD < }
%DIFDELCMD < %%%
%DIF < 
%DIF < 
\DIFdelend \DIFaddbegin \DIFadd{$D_p^2 (1-\phi_s)^3 / 180 \mu \phi_s^2$.
}
\DIFaddend 
...
\item As a result of fluid-structure interaction, the momentum exchange coefficient should be extremely high \DIFdelbegin \DIFdel{(1E15) }\DIFdelend when the solid material points are considered to be zero-porosity/zero-permeability.
...
\caption{GIMP weighting function (red as a convolution of the linear basis shape function (green) and the \DIFdelbegin \DIFdel{characteristic }\DIFdelend \DIFaddbegin \DIFadd{charateristic }\DIFaddend function (blue)}
\label{fig:GIMP}
\end {figure}
%
...
\end {equation}
%
The \DIFdelbegin \DIFdel{characteristic }\DIFdelend \DIFaddbegin \DIFadd{charateristic }\DIFaddend function is the Heaviside function as $\chi_p = 1  \text{  if  } \pmb{x} \in \Omega_p$, otherwise 0 (see Figure \ref{fig:GIMP}). For the interpolation of the centroid of the cell, the linear basis function is used as:
%
\begin{equation}
     S_{ci}  = N_i(\pmb{x_c}) 
\end {equation}
...
%DIFDELCMD < \center
%DIFDELCMD < \includegraphics[scale=0.9]{MPMICE_chart.jpg}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Numerical implementation of MPMICE}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:MPMICE_chart}
%DIFDELCMD < \end {%%%
\DIFdel{sidewaysfigure}%DIFDELCMD < }
%DIFDELCMD < %%%
%DIF < 
%DIF < 
\DIFdelend \subsection{\textsf{Interpolation from Solid Particle to Grid}}
%
%
...
%
%
The nodal external forces $f_{si}^{ext,n}$ and \DIFdelbegin \DIFdel{the nodal frictional forces $f_{si}^{fric}$ from contact between materials }\DIFdelend \DIFaddbegin \DIFadd{extra momentum from contact forces }\DIFaddend are computed here. \DIFaddbegin \DIFadd{The nodal velocity and nodal temperature are applied boundary conditions.}\DIFaddend \\
...
Considering the total fluid \DIFdelbegin \DIFdel{material }\DIFdelend \DIFaddbegin \DIFadd{materal }\DIFaddend volume of a cell is:
%
%
\begin{equation}
\label{V}
...
%
%
Then, we can use \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{he }\DIFaddend Newton-Raphson interation to find the value of $P_{eq}$ which satisfies the equation (\ref{V}, \ref{n}) and each equation of states of each fluid materials.
...
This part compute the viscous shear stress $\Delta (m \pmb{U})_{_{fc},\tau}$ for a single \DIFdelbegin \DIFdel{viscous }\DIFdelend \DIFaddbegin \DIFadd{vicous }\DIFaddend compressible Newtonian fluid and optionally shear stress induced by the turbulent model.
%
%
...
%
%
where $\Delta W_{si}^n = \pmb{\sigma}^\prime : \frac{D_s( \pmb{\epsilon}_s^p)}{Dt} $ is the mechanical work rate computed from the constitutive model with $\pmb{\epsilon}_s^p$ is the plastic strain, $ \Delta W_{fric,i}^n$ is the work rate \DIFdelbegin \DIFdel{computed }\DIFdelend \DIFaddbegin \DIFadd{compted }\DIFaddend from the contact law due to the frictional sliding between solid materials. The heat flux is $\pmb{q}_s = \overline{\rho}_s \beta_s \nabla T_s$ with $\beta_s$ being the thermal conductivity of the solid materials.
...
%
%
Then the boundary condition and contact forces \DIFdelbegin \DIFdel{$f_{si}^{fric}$ }\DIFdelend are applied to the nodal velocity and the acceleration is modified by
%
%
\begin{equation}
...
All input files and the analytical calculations in this section are provided in the Github repository \DIFdelbegin \DIFdel{$(https://github.com/QuocAnh90/Uintah_NTNU)$ }\DIFdelend for the reproduction of the numerical results.\\
%_______________________________
...
\caption{\DIFdelbegin \DIFdel{Comparison }\DIFdelend \DIFaddbegin \DIFadd{Comparision }\DIFaddend between analytical model and simulation}
\label{fig:3d}
\end {subfigure}
...
%
%
A common benchmark \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{fo }\DIFaddend a fully saturated porous \DIFdelbegin \DIFdel{media }\DIFdelend \DIFaddbegin \DIFadd{meida }\DIFaddend is the simulation of one-dimensional consolidation. Using the Carman-Kozeny formula, the time-dependent pressure can be \DIFdelbegin \DIFdel{calculated }\DIFdelend \DIFaddbegin \DIFadd{caluated }\DIFaddend as:
%
%
\begin{equation}
...
\end {subfigure}
\caption{\DIFdelbegin \DIFdel{Comparison }\DIFdelend \DIFaddbegin \DIFadd{Comparision }\DIFaddend between MPMICE model and FEM model}
\label{fig:thermal}
\end {figure}
%
%
Another \DIFdelbegin \DIFdel{benchmark }\DIFdelend \DIFaddbegin \DIFadd{benchkmark }\DIFaddend is the thermal induced cavity flow in porous media. Temperature and velocity distributions are calculated for a square non-deformable saturated porous media The top and bottom walls are insulated, and the left and right walls are at fixed \DIFdelbegin \DIFdel{temperature gradient of }\DIFdelend \DIFaddbegin \DIFadd{temperatures differing by }\DIFaddend 1 degree. The fluid motion at stead state are cavity flow due to the temperature induced density variation. \\
...
}\DIFdelend , the proposed model based on the effective stress analysis which allows to analyze the water pressure  and temperature in the debris flow.\\
%
%
\begin{figure}
\center
...
%
%
A non-associated Mohr-Coulomb model is used for the soil. The soil grain has the density of 2650 kg/$m^3$, Young's modulus of 10 kPa and Poisson's ratio of 0.3 and zero cohesion. The mobilized friction angle $\phi^\prime_m$ is governed following the softening curve (see Figure \ref{fig:model}) with the peak friction angle $\phi^\prime_p$ of 45 degrees and the residual friction angle $\phi^\prime_r$ of 10 degrees. The porosity is 0.3 and the average grain size of the soil is around 0.1 $\mu$m to mimic the undrained behavior. The mobilized dilatancy angle is calculated from the \DIFdelbegin \DIFdel{Rowe's stress dilatancy theory  \mbox{%DIFAUXCMD
\cite{Wood} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Row-stress dilatancy }\DIFaddend as follow:
%
%
\begin{equation}
...
%
%
The solid plane is modeled as a rigid body acted as a shaking table. The contact between horizontal plane and the sand is the frictional contact with the friction coefficient of 0.1. No artificial damping is applied in the simulation. The contact between vertical plane and the sand is \DIFdelbegin \DIFdel{considered }\DIFdelend \DIFaddbegin \DIFadd{consdered }\DIFaddend to be smooth with zero friction coefficient. Under gravity, the density of the water at the surface is 999.8 $kg/m^3$ at the pressure of 1 atm. At the top boundary, the air has a density of 1.17 $kg/m^3$ at the atmospheric pressure of 1 atm. At 5 \DIFdelbegin \DIFdel{Celsius }\DIFdelend \DIFaddbegin \DIFadd{Celcius }\DIFaddend degrees, air and water have viscosity of $18.45e^{-3}$  mPa s and 1  mPa s respectively. On all boundary faces, the symmetric boundary condition is imposed, while the \DIFdelbegin \DIFdel{Neumman }\DIFdelend \DIFaddbegin \DIFadd{Neuman }\DIFaddend boundary condition is imposed at the top \DIFdelbegin \DIFdel{boundary for }\DIFdelend \DIFaddbegin \DIFadd{boundaryfor }\DIFaddend pressure (dp/dx = 0 kPa) and density (d$\rho$/dx = 0 $kg/m^3$). The mesh size is 0.25 x 025m with 300852 element cells and 142316 material points. The simulation takes a couple of hours to perform 60 seconds of the simulation using 4096 CPUs. \\
%
\begin{figure}[H]
\center
...
\end {enumerate}
%
Overall, we show the completed process of the earthquake-induced submarine landslides involving (1) earthquake triggering mechanism, (2) the onset of the shear band with the \DIFdelbegin \DIFdel{development }\DIFdelend \DIFaddbegin \DIFadd{delvelopment }\DIFaddend of negative excess pore water pressure, (3) progressive failure mechanism, (4) submarine landslide induced wave to final deposition.
%
%
\begin{figure}[H]
\center
...
%
%
Here the $\pmb{f}_{fric}$ stems from the soil-structure interaction following the contact law between the soil/structure \DIFdelbegin \DIFdel{interfaces}\DIFdelend \DIFaddbegin \DIFadd{interaces}\DIFaddend .
