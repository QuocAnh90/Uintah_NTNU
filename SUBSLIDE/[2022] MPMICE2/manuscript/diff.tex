...
%DIF DEL MPMICE_v3.tex   Mon Sep 26 05:01:59 2022
%DIF ADD MPMICE_v4.tex   Wed Sep 28 10:27:06 2022
...
\end {gathered}
\end {equation}
\DIFaddbegin \DIFadd{Sliding between solid materials also generates dissipation and the work rate generated from the sliding can be calculated as:
%DIF > 
}\begin{equation}
...
%
%
where \DIFdelbegin \DIFdel{$\Delta W_{si}^n = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}_s$ }\DIFdelend \DIFaddbegin \DIFadd{$\Delta W_{si}^n = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}_s^p$ }\DIFaddend is the mechanical work rate computed from the constitutive model with \DIFdelbegin \DIFdel{$\pmb{\epsilon}_s$ is the total strain}\DIFdelend \DIFaddbegin \DIFadd{$\pmb{\epsilon}_s^p$ is the plastic strain, $ \Delta W_{fric,i}^n$ is the work rate compted from the contact law due to the sliding between solid materials}\DIFaddend . The nodal internal temperature is calculated by
\begin{equation}
...
%
\begin{equation}
     \frac{1}{V}\frac{D_s(m_s e_s)}{Dt} = \Delta W_{s} +  \DIFaddbegin \DIFadd{\Delta W_{friction} + }\DIFaddend \nabla \cdot \pmb{q}_s -  \sum q_{sf} 
\end {equation}
%
%
On the right hand side, the terms include the \DIFdelbegin \DIFdel{rate of the mechanical work $\Delta W_{s} = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}_s$ }\DIFdelend \DIFaddbegin \DIFadd{mechanical work rate $\Delta W_{s} = \pmb{\sigma}^\prime : \Delta \pmb{\epsilon}_s^p$ }\DIFaddend computed from the constitutive model with \DIFdelbegin \DIFdel{$\pmb{\epsilon}_s$ is the total strain}\DIFdelend \DIFaddbegin \DIFadd{$\pmb{\epsilon}_s^p$ is the plastic strain, the work rate from relative sliding between solid materials  $\Delta W_{friction}$}\DIFaddend , thermal transport and energy exchange between solid and fluid respectively. The heat flux is $\pmb{q}_s = \overline{\rho}_s \beta_s \nabla T_s$ with $\beta_s$ being the thermal conductivity of the solid materials
%
%
