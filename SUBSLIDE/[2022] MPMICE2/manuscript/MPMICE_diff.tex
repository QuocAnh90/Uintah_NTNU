...
\DIFdelbeginFL \DIFdelFL{$p_f$ 		  			}%DIFDELCMD < &  %%%
\DIFdelFL{$[M/L^2]$ 	}%DIFDELCMD < & %%%
\DIFdelFL{Isotropic pressure}%DIFDELCMD < \\
%DIFDELCMD < %%%
...
\begin{equation}
    \label{mass} 
  m_s   = \int\DIFdelbegin \DIFdel{_{\Omega} }\DIFdelend \DIFaddbegin \DIFadd{_{\Omega_s} }\DIFaddend \rho_s dV = \overline{\rho}_s V, \qquad  m_f   = \int\DIFdelbegin \DIFdel{_{\Omega} }\DIFdelend \DIFaddbegin \DIFadd{_{\Omega_f} }\DIFaddend \rho_f dV = \overline{\rho}_f V
\end{equation}
%
%
...
%
\begin{equation}
     \frac{1}{V} \DIFdelbegin \DIFdel{\frac{D_f(m_f \pmb{U}_f)}{Dt} }\DIFdelend \DIFaddbegin \left[ \DIFadd{\frac{\partial (m_f \pmb{U}_f)}{\partial t} + \nabla \cdot  (m_f }\pmb{U}\DIFadd{_f }\pmb{U}\DIFadd{_f) }\right] \DIFaddend = \DIFdelbegin \DIFdel{-n}\DIFdelend \DIFaddbegin \DIFadd{- \phi_f }\DIFaddend \nabla \DIFdelbegin \DIFdel{P_{eq} }\DIFdelend \DIFaddbegin \DIFadd{p_f  }\DIFaddend +  \nabla \cdot \pmb{\tau}_f + \overline{\rho}_f \pmb{b} +
     \pmb{f}_{d}
\end{equation}
%
%
...
\begin{equation}
    \label{fluidenergy}
     \frac{1}{V} \DIFdelbegin \DIFdel{\frac{D_f(m_f e_f)}{Dt} }\DIFdelend \DIFaddbegin \left[ \DIFadd{\frac{\partial (m_f e_f)}{\partial t} + \nabla \cdot  (m_f e_f }\pmb{U}\DIFadd{_f) }\right]  \DIFaddend = 
    -\overline{\rho}_f \DIFdelbegin \DIFdel{P_{eq}  }\DIFdelend \DIFaddbegin \DIFadd{p_f  }\DIFaddend \frac{D_f\upsilon_f}{Dt} + \pmb{\tau}_f : \nabla \pmb{U}_f + \nabla \cdot \pmb{q}_f + q_{sf}
\end{equation}
%
%
...
(1) \DIFaddbegin \DIFadd{A constitutive equation to describe the stress - strain behaviour of solid phase (computing effective stress $\pmb{\sigma}^\prime$). }\\
\DIFadd{(2) }\DIFaddend Exchange momentum model (computing drag force $\pmb{f}_{d}$). \\
(\DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend ) Energy exchange model (computing temerature exhange term $q_{sf}$). \\
(\DIFdelbegin \DIFdel{3}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend ) Optional turbulent model to compute the viscous shear stress $\pmb{\tau}_f$.\\
(\DIFdelbegin \DIFdel{4) A constitutive equation to describe the stress - strain behaviour of solid phase (computing effective stress $\pmb{\sigma}^\prime$)}\DIFdelend \DIFaddbegin \DIFadd{5) Frictional force $\pmb{f}_{fric}$ for the contact between soil-structure interaction with the friction coefficient $\mu_{fric}$}\DIFaddend . \\
(\DIFdelbegin \DIFdel{5}\DIFdelend \DIFaddbegin \DIFadd{6}\DIFaddend ) An equation of state to establish relations between thermodynamics variables of each fluid materials $[P_{eq}, \overline{\rho}_f, \upsilon_f, T_f, e_f]$. \\
...
%_______________________________
\subsubsection{\DIFdelbegin \DIFdel{Contact law }\DIFdelend \DIFaddbegin \DIFadd{Frictional force }\DIFaddend for soil-structure interaction}
MPMICE includes a contactlaw for the interaction between soil and structure using the first Coulomb friction contact for MPM presented by Bardenhagen et al. (\cite{MPMcontact}).  The magnitude of the friction force at the contact depends on the friction coefficient $\mu_{fric}$ and the normal force  \DIFdelbegin \DIFdel{$f_{norm}$  }\DIFdelend \DIFaddbegin \DIFadd{$ \pmb{f}_{norm}$  }\DIFaddend computed from the projection of the contact force  in the normal direction.
%
\begin{equation}
   \DIFdelbegin \DIFdel{f}\DIFdelend \DIFaddbegin \pmb{f}\DIFaddend _{fric} = \mu_{fric}  \DIFdelbegin \DIFdel{f}\DIFdelend \DIFaddbegin \pmb{f}\DIFaddend _{norm}
\end{equation}
%
%
The contact determines whether the soil is sliding or sticking to the structure by comparing the friction force with the sticking force \DIFdelbegin \DIFdel{$f_{stick}$  }\DIFdelend \DIFaddbegin \DIFadd{$\pmb{f}_{stick}$  }\DIFaddend computed from the projection of the contact force in the tangent direction:
%
\begin{equation}
\DIFdelbegin %DIFDELCMD < \begin{gathered}
%DIFDELCMD < \textrm{   if    }   f_{fric} \geq f_{stick}   \textrm{    no sliding    } \\
%DIFDELCMD < \textrm{    if    }  f_{fric} <  f_{stick}   \textrm{    sliding occurs    }  
...
\caption{\DIFaddFL{GIMP weighting function (red9 as a convolution of the linear basis shape function (green) and the charateristic function (blue)}}
\label{fig:GIMP}
\end{figure}
%DIF > 
\DIFadd{The fluid phases are }\DIFaddend discretized in the grid with the state variables stored at the centroid of the cells $[\rho_{fc},\pmb{U}_{fc},T_{fc},\upsilon_{fc}]$ while the solid phase is discretized in the particles with the state variables $[m_{sp},\pmb{U}_{sp},T_{sp},\pmb{\sigma}^\prime_{sp}]$. \DIFaddbegin \DIFadd{In the Material Point Method, we use the generalized interpolation technique \mbox{%DIFAUXCMD
\cite{GIMP} }\hspace{0pt}%DIFAUXCMD
using the weight function as a convolution of a grid shape function $N_i(\pmb{x})$ in a nodal domain $\Omega_i$ and a characteristic function $\chi_p (\pmb{x})$ in a particle domain $\Omega_p$ with the volume $V_p (\pmb{x})$ as follows:
%DIF > 
%DIF > 
}\begin{equation}
     \DIFadd{S_{ip} = \frac }{\DIFadd{1}}{\DIFadd{V_p}} \DIFadd{\int_{\Omega_i \cap \Omega_p} N_i(\pmb{x}) \chi_p (\pmb{x}) d\pmb{x}
}\end{equation}
%DIF > 
%DIF > 
...
%DIF > 
     ...
     \DIFadd{V_{p} = \int_{\Omega_p} \chi_p (\pmb{x}) d\pmb{x}
}\end{equation}
%DIF > 
\DIFadd{The charateristic function is the Heaviside function as $\chi_p = 1 if \pmb{x} \in \Omega_p, \textsf{otherwise 0} $ (see Figure \ref{fig:GIMP}).}\\
%DIF > 
...
\DIFdelend \DIFaddbegin \textsf{\DIFadd{Compute fluid pressure (implicit scheme)}}\DIFaddend }
\DIFdelbegin \DIFdel{We solve the generalized Poisson's equation below by employing a preconditioned conjugate gradient technique with a multi-grid pre-conditioner
}\DIFdelend \DIFaddbegin \DIFadd{For single phase flow, the increment of the fluid pressure can be computed as:
}\DIFaddend %
\begin{equation}
  \DIFdelbegin %DIFDELCMD < \left(  
%DIFDELCMD <    %%%
\DIFdelend \kappa\DIFdelbegin \DIFdel{- \nabla^c \frac{\delt}{\overline{\rho}_{f,FC}} \cdot \nabla^{FC}
  }%DIFDELCMD < \right) %%%
\DIFdel{\Delta P_c^{n}
  }\DIFdelend \DIFaddbegin \DIFadd{_f \frac{\Delta P}{dt} }\DIFaddend = \DIFdelbegin \DIFdel{- }\DIFdelend \nabla^c \cdot \pmb{U}_{f,FC}\DIFdelbegin \DIFdel{^{L}
}\DIFdelend \DIFaddbegin \DIFadd{^{n+1}
}\DIFaddend \end{equation}
%
\DIFaddbegin \DIFadd{For multi-phase flows, the increment of the fluid pressure of the mixture can be computed as:
}\DIFaddend %
\DIFdelbegin \DIFdel{The advanced }\DIFdelend \DIFaddbegin \begin{equation}
  \DIFadd{\kappa \frac{\Delta P}{dt} = \sum_{f=1}^{N_f}  \nabla^c \cdot (\phi_{f,FC} \pmb{U}_{f,FC})^{n+1}
}\end{equation}
%DIF > 
\DIFadd{where $\kappa =  \sum_{f=1}^{N_f}  (\phi_{f,FC} \kappa_f)$. Then, the }\DIFaddend fluid pressure at cell center is%DIF < 
\DIFaddbegin \DIFadd{:
}
...
\caption{Model schematic  \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{Ali}}\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL }
...
\caption{\DIFdelbeginFL \DIFdelFL{MPMICE2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MPMICE }\DIFaddendFL model}
\end{subfigure}\hfill    
\begin{subfigure}[d]{0.5\linewidth}
\includegraphics[width=\linewidth]{velocity_thermal-1.jpg}
\caption{FEM model \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{Ali}}\hspace{0pt}%DIFAUXCMD
}\DIFaddendFL }
\end{subfigure}
...
%
%
A non-associated Mohr-Coulomb model is used for the \DIFdelbegin \DIFdel{sand}\DIFdelend \DIFaddbegin \DIFadd{soil}\DIFaddend . The soil grain has the density of 2650 kg/\DIFdelbegin \DIFdel{$m_3$}\DIFdelend \DIFaddbegin \DIFadd{$m^3$}\DIFaddend , Young's modulus of 10 kPa and Poisson's ratio of 0.3 and zero cohesion. The \DIFdelbegin \DIFdel{firction angle is mobilized }\DIFdelend \DIFaddbegin \DIFadd{mobilized friction angle $\phi^\prime_m$ is governed }\DIFaddend following the softening curve (see Figure \ref{fig:model}) with the peak friction angle \DIFaddbegin \DIFadd{$\phi^\prime_p$ }\DIFaddend of 45 degrees and the residual friction angle \DIFaddbegin \DIFadd{$\phi^\prime_r$ }\DIFaddend of 10 degrees. The average grain size of the \DIFdelbegin \DIFdel{clay }\DIFdelend \DIFaddbegin \DIFadd{soil }\DIFaddend is around 0.1 $\mu$m to mimic the undrained behavior. The mobilized dilatancy angle is calculated from the Row-stress dilatancy as follow:
%
%
\begin{equation}
...
\begin{figure}[H]
\center
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=0.5]{landslide_vel.jpg}
%DIFDELCMD < %%%
...
%
%
Before deriving the governing equation, we \DIFdelbegin \DIFdel{give some definition (following \mbox{%DIFAUXCMD
\cite{Kashiwa} }\hspace{0pt}%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{define the Lagrangian derivative for a state variable $f$ as:
%DIF > 
%DIF > 
}\begin{equation}
\DIFadd{\frac{D_f f}{Dt} =  \frac{\partial f}{\partial t} + \pmb{U_f} \cdot \nabla f \qquad
\frac{D_s f}{Dt} =  \frac{\partial f}{\partial t} + \pmb{U_s} \cdot \nabla f
}\end{equation}
%DIF > 
%DIF > 
\DIFadd{we use some definition following \mbox{%DIFAUXCMD
\cite{Kashiwa} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend as below:
%
%
...
\DIFdelbegin %DIFDELCMD < \begin{gathered}
%DIFDELCMD <    \frac{1}{V} \frac{D_f V}{Dt} = \frac{1}{V} \left( \left[ \frac{\partial V}{\partial p} \right] \frac{D_f p}{D t} + \left[ \frac{\partial V}{\partial T} \right] \frac{D_f T}{D t} \right) = \frac{1}{V} \left( -\kappa \frac{D_f p}{D t} + \alpha \frac{D_f T}{D t} \right) \\
...
\DIFdel{%DIFDELCMD < \label{solidvolumerate}%%%
...
\begin{equation}
\DIFdelbegin %DIFDELCMD < \begin{gathered}
%DIFDELCMD <     0 = \frac{\partial }{\partial t} (n - \sum_{f=1}^{N_f} \upsilon_f \rho_f) =  \frac{\partial n}{\partial t} - \sum_{f=1}^{N_f} (\rho_f\frac{\partial \upsilon_f}{\partial t} + \upsilon_f \frac{\partial \rho_f}{\partial t}) \\
%DIFDELCMD < = \frac{\partial n}{\partial t} - \sum_{f=1}^{N_f} (\rho_f\frac{D_f \upsilon_f}{Dt} + \nabla \cdot (\upsilon_f \rho_f \pmb{U}_f))\\
%DIFDELCMD < \label{equilP}
%DIFDELCMD < \end{gathered}%%%
\DIFdelend \DIFaddbegin \begin{gathered}
...
%DIF < 
    ...
    \DIFdel{\frac{D_f \upsilon_f}{Dt} = \upsilon_f (\kappa_f \frac{D_f P_{eq}}{Dt} + \alpha_f  \frac{D_f T_f}{Dt})
%DIFDELCMD < \label{specificvolume}%%%
}\end{displaymath}%DIFAUXCMD
%DIF < 
%DIF < 
\DIFdel{Then }\DIFdelend \DIFaddbegin \DIFadd{Putting the equation (\ref{fluidvolumerate}) into }\DIFaddend equation (\ref{equilP}) \DIFdelbegin \DIFdel{becomes:
...
\begin{equation}
    ...
    \nabla \cdot (\DIFdelbegin \DIFdel{-np}\DIFdelend \DIFaddbegin \DIFadd{-\phi}\DIFaddend _f \DIFaddbegin \DIFadd{p_f}\DIFaddend \pmb{I}) + \nabla \cdot \pmb{\tau}_f + \overline{\rho}_f \pmb{b} + \pmb{f}_{d} + \pmb{f}_b
\end{equation}
%
%
...
\begin{equation}
    \label{fluidmom}
     \frac{1}{V}\frac{D_f(m_f \pmb{U}_f)}{Dt} =  \DIFdelbegin \DIFdel{-n}\DIFdelend \DIFaddbegin \DIFadd{\frac{1}{V} }\left[ \DIFadd{\frac{\partial (m_f \pmb{U}_f)}{\partial t} + }\DIFaddend \nabla \DIFaddbegin \DIFadd{\cdot  (m_f }\pmb{U}\DIFadd{_f }\pmb{U}\DIFadd{_f) }\right] \DIFadd{= -\phi_f \nabla }\DIFaddend p_f + \nabla \cdot \pmb{\tau}_f + 
