...
A representative element volume $\Omega$ is decomposed by two domains: solid domain $\Omega_s$ and fluid domain $\Omega_f$. Then, the two domains are homogenized into two overlapping continua. Considering the true (or Eulerian) porosity $n=\Omega_f/\Omega$ of the representative element volume, the average density of solid and fluid phase are defined as:\\
%
%
\begin{equation}
    \label{density} 
  \overline{\rho}_s   = (1-n) \rho_s, \qquad  \overline{\rho}_f   = n \rho_f \quad
\end{equation}
%
%
...
Currently, the energy exchange coefficient is assumed to be constant for the sake of simplicity. For the momentum exchange, we can either select the Darcy's law for laminar flow or other models for non laminar flow
\subsubsection{Darcy law}
The drag force following the Darcy law is given by
%
%
\begin{equation}
     \pmb{f}_{fs} = n^2 \frac{\mu}{\kappa} (\pmb{U}_s - \pmb{U}_f)  
\end{equation}
%
%
where $\mu$ is the fluid viscosity, $\kappa$ is the intrinsic permeability of soil which can be calculated using Kozeny-Carman equation for sand
%
%
\begin{equation}
...
%
%
where $D_p$ is average dimameter of sand grains and $A$ is empirical parameter considering the tortuosity effect. 
...
\begin{equation}
    ...
    \nabla \cdot (-np_f\pmb{I}) + \nabla \cdot \pmb{\tau}_f + \overline{\rho}_f \pmb{b} + \pmb{f}_{sf} + \pmb{f}_b
\end{equation}
%
%
...
\begin{equation}
\label{e1}
     \frac{1}{V}\frac{D_f(m_f (e_f+0.5\pmb{U}_f^2))}{Dt} = \nabla \cdot (-np_f\pmb{I}) \cdot \pmb{U}_f + \nabla \cdot \pmb{q}_f + (\overline{\rho}_f \pmb{b}) \cdot \pmb{U}_f + \pmb{f}_{sf} \cdot \pmb{U}_f + q_{sf}
\end{equation}
%
%
...
\begin{gathered}
     ...
     + \nabla \cdot \pmb{q}_f + \nabla \cdot \pmb{q}_s + \pmb{f}_{sf} \cdot (\pmb{U}_f - \pmb{U}_s)  
\end{gathered}
\end{equation}
%
%
