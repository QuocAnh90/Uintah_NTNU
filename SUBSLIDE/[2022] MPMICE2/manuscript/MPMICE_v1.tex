...
(2) Exchange momentum model (computing drag force $\pmb{f}_{d}$). \\
(3) Energy exchange model (computing temerature exhange term $q_{sf}$). \\
(4) Optional turbulent model to compute the viscous shear stress $\pmb{\tau}_f$.\\
(5) Frictional force $\pmb{f}_{fric}$ for the contact between soil-structure interaction with the friction coefficient $\mu_{fric}$. \\
...
\caption{GIMP weighting function (red9 as a convolution of the linear basis shape function (green) and the charateristic function (blue)}
\label{fig:GIMP}
\end{figure}
%
...
\end{equation}
%
The charateristic function is the Heaviside function as $\chi_p = 1 if \pmb{x} \in \Omega_p, \textsf{otherwise 0} $ (see Figure \ref{fig:GIMP}).\\
%
